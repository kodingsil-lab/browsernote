...
# ============================================================
# BrowserNote - STAGE 06
# LOCAL RECOVERY / BROWSER DRAFT PROTECTION
...
$BackupDir   = Join-Path $ProjectRoot "writable\backups\stage06_$Stamp"
...
if (-not (Test-Path $JsPath)) {
    Fail "browsernote.js tidak ditemukan."
}

$existing = [System.IO.File]::ReadAllText($JsPath)
...
foreach ($marker in $stage05Markers) {
    if (-not $existing.Contains($marker)) {
        Fail "Stage 05 belum lengkap. Marker hilang: $marker"
    }
}

Ok "Stage 05 terverifikasi"
...
Step "3. INSTALL LOCAL RECOVERY JAVASCRIPT"
...
(() => {
    ...
    function shouldRecoverDraft(draft, note) {
        ...
        /*
         * Safe automatic recovery when the server row has not changed
         * since this draft started.
         */
        if (
            draft.baseUpdatedAt
            && note.updated_at
        ...
        /*
         * Drafts created before a reliable base timestamp are also
         * recovered because preserving unsaved text is the priority.
         */
        ...
        /*
         * A different server timestamp can mean another tab saved a
         * newer version. Ask before replacing what is on the server.
         */
    ...
    async function performSave({
        ...
        /*
         * Capture local draft synchronously immediately before the
         * network request. If the request fails, this remains.
         */
        clearLocalDraftTimer();
        persistLocalDraftNow();
    ...
    function bindUI() {
        ...
        window.addEventListener(
            ...
            (event) => {
                /*
                 * localStorage write is synchronous. Capture the latest
                 * editor state before the document is allowed to close.
                 */
                if (state.dirty) {
                    clearLocalDraftTimer();
                    persistLocalDraftNow();
    ...
    /*
     * Small diagnostic surface for Stage 06 and future maintenance.
     * No server data is exposed beyond this browser tab.
     */
...
Ok "Local recovery JavaScript dipasang"

Step "4. STATIC RECOVERY VERIFICATION"
...
foreach ($fragment in $requiredFragments) {
    if (-not $installed.Contains($fragment)) {
        Fail "Komponen local recovery tidak ditemukan: $fragment"
    }
...
if ($response.Content -notlike '*browsernote.draft.v1.*') {
    Fail "Apache masih menyajikan JavaScript lama."
}

Ok "JavaScript Local Recovery tersedia melalui Apache"
...
Write-Host "Server save   : SQLite autosave 700 ms" -ForegroundColor White
Write-Host "Local draft   : localStorage 180 ms" -ForegroundColor White
Write-Host "Crash recovery: PASS" -ForegroundColor White
Write-Host "Draft cleanup : PASS" -ForegroundColor White
Write-Host "Conflict guard: PASS" -ForegroundColor White
Write-Host "API           : PASS" -ForegroundColor White
Write-Host ""
Write-Host "Uji manual Stage 06:" -ForegroundColor Green
Write-Host "1. Buka satu catatan lalu ketik teks baru." -ForegroundColor White
Write-Host "2. SEBELUM 700 ms selesai, tekan F5/refresh." -ForegroundColor White
Write-Host "3. Jika browser memberi peringatan perubahan belum tersimpan, pilih Leave/Reload." -ForegroundColor White
Write-Host "4. Setelah halaman terbuka lagi, draft lokal harus dipulihkan." -ForegroundColor White
Write-Host "5. Tunggu sampai status menjadi Tersimpan otomatis." -ForegroundColor White
Write-Host "6. Refresh lagi. Catatan harus berasal dari SQLite dan tidak memunculkan recovery ulang." -ForegroundColor White
Write-Host ""
Write-Host "Tes tambahan opsional:" -ForegroundColor Green
Write-Host "1. Ketik lalu matikan Apache sebelum autosave server selesai." -ForegroundColor White
Write-Host "2. Draft lokal tetap tersimpan di browser." -ForegroundColor White
Write-Host "3. Hidupkan Apache, buka kembali BrowserNote, dan draft akan dipulihkan." -ForegroundColor White
Write-Host ""
Write-Host "Berikutnya : Stage 07 - Folder management" -ForegroundColor Green
